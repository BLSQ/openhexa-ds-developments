...
1. Create an instance of a matcher. This is the matcher that will be used to compare the different levels of the pyramids. If no matcher is provided, a default one will be used. The file `pyramid_matcher/matchers.py` contains several matchers that you can use depending on your needs.
...
reference_pyramid = pl.DataFrame(...)  # your reference pyramid
candidate_pyramid = pl.DataFrame(...)  # your candidate pyramid

matcher = FuzzyMatcher(threshold=80, scorer_name="wratio")
...
| Province 2   | 2          | Another 2                 | Sector A     | 22         |
| Province 3   | 3          | Another 3                 | District30   | 31         |
| Territoire 5 | 5          | Another 5                 | District 50  | 50         |


then the `matched_data` output will look like:

| candidate_level_2_name | candidate_level_2_id | candidate_level_2_another_attribute | reference_level_2_name | reference_level_2_id | score_level_2 | repeated_matches_level_2 | candidate_level_3_name | candidate_level_3_id | reference_level_3_name | reference_level_3_id | reference_level_3_other_attribute | score_level_3 | repeated_matches_level_3 |
|------------------------|----------------------|--------------------------------------|------------------------|----------------------|---------------|--------------------------|------------------------|----------------------|------------------------|----------------------|-----------------------------------|---------------|--------------------------|
| Province 1             | 1                    | Another 1                            | Province 1             | 100                  | 100           | False                    | District 10            | 10                   | District 10            | 1000                 | Attribute 1000                    | 100           | False                    |
| Province 2             | 2                    | Another 2                            | Province 2             | 200                  | 100           | False                    | District 20            | 20                   | District 20            | 2000                 | Attribute 2000                    | 100           | False                    |
| Province 2             | 2                    | Another 2                            | Province 2             | 200                  | 100           | False                    | District 21            | 21                   | District 21            | 2001                 | Attribute 2001                    | 100           | False                    |
| Province 3             | 3                    | Another 3                            | Province 3             | 300                  | 100           | False                    | District30             | 31                   | District 30            | 3000                 | Attribute 3000                    | 90            | False                    |
...
- `repeated_matches_level_*` columns indicate whether the match for that level is repeated (i.e., if the same reference level is matched to multiple candidate levels).


The output `matched_data_simplified` will be:
| candidate_level_2_name | candidate_level_2_id | candidate_level_2_another_attribute | reference_level_2_name | reference_level_2_id | candidate_level_3_name | candidate_level_3_id | reference_level_3_name | reference_level_3_id | reference_level_3_other_attribute |
|------------------------|----------------------|-------------------------------------|------------------------|----------------------|------------------------|----------------------|------------------------|----------------------|-----------------------------------|
| Province 1             | 1                    | Another 1                           | Province 1             | 100                  | District 10            | 10                   | District 10            | 1000                 | Attribute 1000                    |
| Province 2             | 2                    | Another 2                           | Province 2             | 200                  | District 20            | 20                   | District 20            | 2000                 | Attribute 2000                    |
| Province 2             | 2                    | Another 2                           | Province 2             | 200                  | District 21            | 21                   | District 21            | 2001                 | Attribute 2001                    |
| Province 3             | 3                    | Another 3                           | Province 3             | 300                  | District30             | 31                   | District 30            | 3000                 | Attribute 3000                    |
...
|--------------|------------|--------------|------------|-------------------------|-----------------|
| Province 4   | 400        | District 40  | 4000       | Attribute 4000          | level_2         |
| Province 1   | 100        | District 11  | 1001       | Attribute 1001          | level_3         |


The output `candidate_not_matched` will be:
| level_2_name | level_2_id | level_2_another_attribute | level_3_name | level_3_id | unmatched_level |
|--------------|------------|---------------------------|--------------|------------|-----------------|
| Territoire 5 | 5          | Another 5                 | District 50  | 50         | level_2         |
| Province 2   | 2          | Another 2                 | Sector A     | 22         | level_3         |
...
- `matched_data`: This is a DataFrame that contains the matched data between the reference and candidate pyramids. It includes the matched levels, the attributes of the matched levels, the similarity score of the matches and whether the matches are repeated or not. (See section 2.5 for more details about how the matches are done, including information about what is considered an attribute and what is considered a repeated match).
...
- The columns that will be matched against each others are named following the pattern `level_{level_number}{matching_col_suffix}`. For example, if the `matching_col_suffix` is `_name`, the columns to do the matching with should be named `level_1_name`, `level_2_name`, etc. If the `matching_col_suffix` is `_geometry`, the columns to do the matching with should be named `level_1_geometry`, `level_2_geometry`, etc. The levels that will be matched will be those that have a common column in both pyramids.

- The characteristics of the levels that are to be included in the output (which we have been calling attributes) should have the prefix `level_{level_number}_{attribute_name}`. For example, if we want the id's to be including in the output, and we are matching the names against each other, the columns with the id's should be named `level_1_id`, `level_2_id`, etc. If we want the geometries to be included in the output, the columns with the geometries should be named `level_1_geometry`, `level_2_geometry`, etc. All of the attributes will be included in the output, even if they are included only in some levels or only in the reference or candidate pyramids. So if only the reference pyramid has a column named `level_4_antenne`, this column will still appear in the output, linked to the reference pyramid column  `level_4_{matching_col_suffix}`
...
The `PyramidMatcher` class is designed to be flexible and can be used with different types of matchers, which are responsible for calculating the similarity score between the levels of the pyramids and determining whether a match is considered a match or not based on a minimum similarity score threshold. The file `pyramid_matcher/matchers.py` contains several matchers that you can use depending on your needs.

For now, only the `FuzzyMatcher` is fully implemented. `SentenceTransformerMatcher` and `GeometryMatcher` exist in `pyramid_matcher/matchers.py` but are not yet implemented.

### 3.1 FuzzyMatcher
The `FuzzyMatcher` uses fuzzy string matching (via `rapidfuzz`) to compute similarity scores between level names. It accepts two parameters:

- `threshold` (default: `80`): The minimum score (0–100) for a match to be accepted. A higher value means stricter matching; a lower value accepts more matches at the risk of false positives. For example, `"District30"` vs `"District 30"` scores ~90. Setting the threshold to 80 would accept this match, while setting it to 95 would reject it.
- `scorer_name` (default: `"wratio"`): The scoring algorithm. Options: `"ratio"`, `"partial_ratio"`, `"token_sort_ratio"`, `"token_set_ratio"`, `"wratio"`. `"wratio"` is a weighted combination and is generally the best all-around choice.
